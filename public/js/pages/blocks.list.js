...
class BlocksList {
    ...
    _initProgressBars() {
        ...
        document.querySelectorAll(".progress-bar-line").forEach((el, i) => {
            ...
            const color =
                Globals[el.getAttribute("data-color")] || Globals.primary;
            const trailColor =
                Globals[el.getAttribute("data-trail-color")] ||
                Globals.separator;
        ...
        for (let i = 0; i < this._progressBars.length; i++) {
            this._progressBars[i].animate(
                this._progressBars[i]._opts.val /
                    this._progressBars[i]._opts.max
            );
        }
    }
