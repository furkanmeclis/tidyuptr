...
class ClassyDataTable {
    ...
    _createInstance() {
        ...
        this._datatable = jQuery("#datatableRows2").DataTable({
            ...
            initComplete: function (settings, json) {
                _this._saveSettings();
                _this.rowIndex.forEach(function(el) {
                    _this._datatable.rows(el).select();
                });
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row, meta) {
                        if (row.selected === true) {
                            _this.rowIndex.push(meta.row);
                            return (
                                '<div class="form-check float-end mt-1"><input type="checkbox" checked class="form-check-input" value="' +
                                row.id +
                                '" name="student_id_tmp"></div>'
                            );
                        } else {
                            return (
                                '<div class="form-check float-end mt-1"><input type="checkbox" class="form-check-input" value="' +
                                row.id +
                                '" name="student_id_tmp"></div>'
                            );
                        }
    ...
    _saveSettings = async () => {
        const form = document.getElementById("createClass");
        if (!form) {
            return;
        }
        ...
        if(!form.hasAttribute('data-event-listener')){
            form.addEventListener("submit", async (event) => {
                form.setAttribute('data-event-listener', 'true');
                event.preventDefault();
                ...
                if (jQuery(form).valid()) {
                    ...
                    await selected.every(function (rowIdx, tableLoop, rowLoop) {
                        ...
                        selectedStudents.push(data.id);
                    });
                    let formData = $('#createClass').serializeArray();
                    selectedStudents.forEach((item) => {
                        formData.push({name: 'student_id[]', value: item});
                    });
                    $.ajax({
                        url: document.getElementById("createClass").getAttribute("action"),
                        type: "PUT",
                        data: formData,
