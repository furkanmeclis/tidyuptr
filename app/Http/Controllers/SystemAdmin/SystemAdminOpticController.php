...
class SystemAdminOpticController extends Controller
{
    public function index()
    {
        $schemes = OpticalParameter::all();
        return view('systemAdmin.optic.all',compact('schemes'));
    }

    public function create()
    {
        $lessons = Lesson::all();
        return view('systemAdmin.optic.create',compact('lessons'));
    }
    ...
    public function uploadFmt(Request $request)
    {
        try{
            ...
            ],"url" => route('systemAdmin.optic.store')]);
        }catch (\Exception $e){
        ...
        try{
            if($request->hasFile('file')){
                ...
                if($optic->save()){
                    ...
                    return response()->json(['status' => true, 'message' => 'Optik Şeması Başarıyla Yüklendi',"url"=>route('systemAdmin.optic.index')]);
                }else {
                    if ($path){
                        Storage::delete($path);
                    }
    ...
    public function edit($id)
    {
        ...
        return view('systemAdmin.examScheme.edit',compact('scheme'));
    }
